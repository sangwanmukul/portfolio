...
import { FaDocker, FaGitAlt, FaGithub, FaJava, FaLinkedin, FaPython, FaReact, FaServer } from 'react-icons/fa'
...
export default function HeroSection() {
  ...
  const [activeTab, setActiveTab] = useState<'skills' | 'about' | 'interests'>('skills')
    ...
    const handleTyping = () => {
      ...
      if (!isDeleting) {
        ...
        if (currentText === fullWord) {
          timer = window.setTimeout(() => setIsDeleting(true), 1600)
          return
        }
        timer = window.setTimeout(handleTyping, 80)
      } else {
        // Deleting
        setCurrentText(fullWord.substring(0, currentText.length - 1))
        if (currentText === '') {
          ...
          timer = window.setTimeout(handleTyping, 300)
          return
        }
        timer = window.setTimeout(handleTyping, 45)
      }
    }
    timer = window.setTimeout(handleTyping, 100)
    return () => window.clearTimeout(timer)
  }, [currentText, isDeleting, currentWordIndex])

  return (
    <section id="home" className="relative isolate overflow-hidden px-4 py-20 sm:px-6 lg:px-8 lg:py-32">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.1fr_0.9fr] pb-12">
        <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-cyan-600 dark:text-cyan-300">
            <Sparkles size={14} className="animate-pulse text-cyan-500" />
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="relative">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white/70 p-6 shadow-[0_30px_80px_rgba(15,23,42,0.16)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/70">
            ...
            <div className="mx-auto mb-6 relative flex h-28 w-28 items-center justify-center">
              <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-blue-600 via-violet-600 to-cyan-400 animate-spin opacity-75 blur-[2px]" style={{ animationDuration: '8s' }} />
              <div className="relative h-[104px] w-[104px] overflow-hidden rounded-full border-4 border-white bg-slate-100 dark:border-slate-900 dark:bg-slate-800 shadow-lg">
                <img src="/mukul.jpg" alt="Mukul Sangwan" className="h-full w-full object-cover" />
              </div>
            </div>

            <div className="mt-8 relative rounded-[1.5rem] border border-slate-200/70 bg-slate-950 p-6 text-slate-100 dark:border-slate-800 min-h-[220px]">
              ...
              {/* Mock IDE Tabs */}
              <div className="absolute left-20 top-4 flex gap-1 font-mono text-[10px]">
                {(['skills', 'about', 'interests'] as const).map((tab) => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === 'skills' ? 'Skills.ts' : tab === 'about' ? 'About.json' : 'Interests.txt'}
                  </button>
                ))}
              </div>

              <div className="mt-6 font-mono text-xs leading-6 min-h-[140px]">
                {activeTab === 'skills' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    <p className="text-cyan-400">const skills = [</p>
                    <p className="ml-4 text-slate-300">'Java', 'Spring Boot', 'Python', 'React',</p>
                    <p className="ml-4 text-slate-300">'Machine Learning', 'Docker', 'MySQL'</p>
                    <p className="text-cyan-400">]</p>
                    <p className="text-cyan-400">const config = {'{'}</p>
                ...
                {activeTab === 'about' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    ...
                    <p className="ml-4"><span className="text-cyan-400">"name"</span>: <span className="text-amber-300">"Mukul Sangwan"</span>,</p>
                    <p className="ml-4"><span className="text-cyan-400">"role"</span>: <span className="text-amber-300">"Software Developer"</span>,</p>
                    <p className="ml-4"><span className="text-cyan-400">"education"</span>: <span className="text-amber-300">"VIT Chennai"</span>,</p>
                    <p className="ml-4"><span className="text-cyan-400">"focus"</span>: <span className="text-amber-300">"Distributed Systems & ML"</span></p>
                    <p className="text-violet-400">{'}'}</p>
                  </motion.div>
                )}

                {activeTab === 'interests' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="space-y-1 text-slate-300 font-mono">
                    <p className="text-slate-500">// My Interests</p>
                    <p>1. Designing distributed systems</p>
                    <p>2. Adversarial CAPTCHA ML defense</p>
                    <p>3. Dynamic UI UX development</p>
                    <p>4. Reading engineering publications</p>
                  </motion.div>
                )}
              </div>
            </div>
              ...
              {floatingIcons.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex flex-col items-center gap-2 rounded-2xl border border-slate-200/70 bg-slate-50/80 p-3 text-xs font-medium text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 dark:text-slate-200 transition duration-300 hover:scale-105 hover:border-cyan-400/50 hover:shadow-md"
                  >
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5 opacity-60">
        <span className="text-[10px] uppercase tracking-[0.25em] text-slate-400 dark:text-slate-500">Scroll Down</span>
        <motion.div
          animate={{ y: [0, 8, 0] }}
          transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
          className="flex h-7 w-4 justify-center rounded-full border-2 border-slate-400 dark:border-slate-600 p-1"
        >
